...
class zubolog {
    ...
    private function getZubologArray($dir, $perpage, $sort = true, $current) {
        ...
        if(is_array($dir)) {
          foreach ($dir as $item) {
            if($this->zubologGetCaption($item, $current) !== null) {
              $captionArr = $this->zubologGetCaption($item, $current);
            } else {
              $captionArr = null;
            }
            foreach(glob($current . '/'. $item . '*') as $filepath) {
              $caption = '';
              $pregmatch = preg_replace('/filename/', '$1', $this->thumbnail_name);
                ...
                $filenameArray[] = array($item, pathinfo($filepath)['basename'], $caption);
            }
          }
          ...
          foreach(glob($current . '/'. $dir . '*') as $filepath) {
            ...
            $pregmatch = preg_replace('/filename/', '$1', $this->thumbnail_name);
              ...
              $filenameArray[] = array($dir, pathinfo($filepath)['basename'], $caption);
          }
        }

        if ($sort == true) {
          $retFilenameArray = array_reverse($filenameArray);
        } else {
          $retFilenameArray = $filenameArray;
        }
